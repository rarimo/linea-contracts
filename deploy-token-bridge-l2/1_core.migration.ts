...
export = async (deployer: Deployer) => {
  ...
  const securityCouncilAddress = config.L2_TOKEN_BRIDGE_SECURITY_COUNCIL;
  ...
  console.log(`Deploying TokenBridge on L2, using L2_RESERVED_TOKEN_ADDRESSES environment variable`);

  const initData = TokenBridge__factory.createInterface().encodeFunctionData("initialize", [
    {
      defaultAdmin: securityCouncilAddress,
      messageService: config.L2_MESSAGE_SERVICE_ADDRESS,
      tokenBeacon: beaconProxyAddress,
      sourceChainId: chainId,
      targetChainId: config.REMOTE_CHAIN_ID,
      reservedTokens: config.L2_RESERVED_TOKEN_ADDRESSES,
  ...
  await Reporter.reportContractsMD(["L2 Token Bridge", await tokenBridgeProxy.getAddress()]);
};
